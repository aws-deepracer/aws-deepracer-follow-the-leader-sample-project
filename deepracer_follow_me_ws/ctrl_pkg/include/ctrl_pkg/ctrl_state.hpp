...
namespace SysCtrl {
    ...
    class FollowMeDriveCtrl : public CtrlStateBase
    {
    /// Control state for follow me driving.
    public:
        /// @param nodeHdl Reference to the node handler of the control node.
        /// @param subName Topic to subscribe to for servo messages.
        FollowMeDriveCtrl(std::shared_ptr<rclcpp::Node> ctrlNodePtr, const std::string &subName);
        virtual ~FollowMeDriveCtrl() = default;
