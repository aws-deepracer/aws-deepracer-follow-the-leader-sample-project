...
@VEHICLE_CONTROL_BLUEPRINT.route("/api/followme_max_speed", methods=["PUT", "POST"])
def max_followme_speed():
    """API to call the SetFollowMeMaxSpeedSrv service to set the throttle scale in the
       followme mode.
    ...
    """
    webserver_node = webserver_publisher_node.get_webserver_node()
    followme_speed = request.json.get("throttle")
    if followme_speed is None:
        return jsonify({"success": False, "reason": "value must be set."})
    webserver_node.get_logger().info(f"Setting max followme speed to {followme_speed}%")
    try:
        set_speed_req = SetFollowMeMaxSpeedSrv.Request()
        set_speed_req.max_speed_pct = float(followme_speed / 100)
        set_speed_res = call_service_sync(webserver_node.set_followme_max_speed_cli, set_speed_req)
        if set_speed_res and (set_speed_res.error == 0):
            return jsonify({"success": True})
        else:
            return jsonify(success=False, reason="Failed to call the SetFollowMeMaxSpeed service")
    except Exception as ex:
        webserver_node.get_logger().error(f"Unable to reach SetFollowMeMaxSpeed server: {ex}")
        return jsonify(success=False, reason="Unable to reach SetFollowMeMaxSpeed server")
