...
from deepracer_interfaces_pkg.msg import (ObjectDetectionDeltaMsg,
                                          ServoCtrlMsg)
from deepracer_interfaces_pkg.srv import SetFollowMeMaxSpeedSrv
from follow_me_navigation_pkg import (constants,
                                      utils)


class FollowMeNavigationNode(Node):
    """Node responsible for deciding the action messages (servo control messages specifically angle
       and throttle) to be sent out using the detection deltas from object_detection_node.
    """

    def __init__(self, qos_profile):
        """Create a FollowMeNavigationNode.
        """
        super().__init__('follow_me_navigation_node')
        self.get_logger().info("follow_me_navigation_node started.")
        ...
        self.set_max_speed_service = self.create_service(SetFollowMeMaxSpeedSrv,
    ...
    def set_max_speed_cb(self, req, res):
        ...
        Args:
            req (SetFollowMeMaxSpeedSrv.Request): Request object with the updated
                                                  max speed percentage.
            res (SetFollowMeMaxSpeedSrv.Response): Response object with error(int) flag
                                                   indicating successful max speed pct
                                                   update.

        Returns:
            SetFollowMeMaxSpeedSrv.Response: Response object with error(int) flag indicating
...
def main(args=None):
    ...
    try:
        follow_me_navigation_node = FollowMeNavigationNode(qos)
        ...
        def signal_handler(signum, frame):
            ...
            """
            follow_me_navigation_node.get_logger().info("Signal Handler initiated")
            follow_me_navigation_node.thread_shutdown()
            follow_me_navigation_node.wait_for_thread()

        # Register SIGINT handler
        signal.signal(signal.SIGINT, signal_handler)
        rclpy.spin(follow_me_navigation_node, executor)
    except Exception as ex:
        follow_me_navigation_node.get_logger().error(f"Exception in FollowMe Navigation Node: {ex}")
        follow_me_navigation_node.destroy_node()
        rclpy.shutdown()
